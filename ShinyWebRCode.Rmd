...
# Update team selection choices in alphabetical order
observe({
...
# Calculate league table based on selected game week using SQL
league_table_data <- reactive({
  game_week <- input$game_week
  
  # SQL query to calculate points, wins, draws, losses, goal difference, and games played up to the selected game week
  query <- paste0("
    WITH GameResults AS (
      SELECT 
        HomeTeam AS Team, 
  ...
  # Execute the SQL query to get the league table
  league_table_df <- dbGetQuery(con, query)
  
  # Add position based on sorted PTS and goal difference
  league_table_df$POS <- seq_len(nrow(league_table_df))
  league_table_df
})

# Render league table output
...
# Display the last `n` games for a selected team using SQL for data retrieval and formatting
output$last_games_table <- renderTable({
  ...
  # SQL query to fetch the last `n` games with relevant information
  query <- paste0("
    SELECT 
      GW AS GameWeek, 
      CASE 
  ...
  # Execute query and fetch the last games data
  ...
  observe({
  # SQL query to fetch distinct team names from match_status, sorted alphabetically
...
output$pointsPlot <- renderPlot({
  # SQL query to retrieve data for selected teams and season, including point summary
  query <- paste0(
...
# Render the summary table for selected teams by season
output$pointsSummaryTable <- renderTable({
  ...
  # Retrieve the selected season range
  season_min <- input$season_range[1]
  season_max <- input$season_range[2]
  
  # Modify summary query to filter based on season range
  ...
  # Execute the query and retrieve the summary data
  summary_data <- dbGetQuery(con, summary_query)
  
  # Return the summary data for rendering in the table
  summary_data
})
